...
router.post("/register", async (req, res) => {
  if (!req.body.firstname)
    return res.status(400).send("Firstname is not define");
  ...
  if (!req.body.number) return res.status(400).send("Number not define");
...
router.put("/update", auth, async (req, res) => {
  ...
  User.changeProfile(
    { password, firstname, lastname, address, number },
    req.user._id
  )
    .then((user) => {
